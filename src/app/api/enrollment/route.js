...
export const POST = async (request) => {
  ...
  const fStudent = DB.students.find((x) => x.studentId === studentId);
  const fCourse = DB.courses.find((x) => x.courseNo === courseNo);
  if (!fStudent || !fCourse) {
    return NextResponse.json(
      {
        ok: false,
  ...
  const fEnroll = DB.enrollments.find(
...
export const DELETE = async (request) => {
  ...
  const fEnroll = DB.enrollments.findIndex(
    (x) => x.studentId === studentId && x.courseNo === courseNo
  );
  if (fEnroll === -1)
    return NextResponse.json(
  ...
  if (fEnroll) {
    DB.enrollments.splice(fEnroll, 1);
  }
